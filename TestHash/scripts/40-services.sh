...
check_http_contract() {
  ...
  # The legacy API advertises /static/7zr.bin to Linux agents.  A SPA fallback
  # can otherwise return index.html with HTTP 200 and leave the agent retrying
  # utility preparation forever, so validate both the route and ELF signature.
  HEALTH_STATIC_CODE="$(curl -sS -o /dev/null -w '%{http_code}' --max-time 8 \
      "http://127.0.0.1:${INTERNAL_PORT}/static/7zr.bin" 2>/dev/null || true)"
  [[ "${HEALTH_STATIC_CODE}" =~ ^[0-9]{3}$ ]] || HEALTH_STATIC_CODE=000
  HEALTH_STATIC_SIGNATURE="$(
    curl -fsS --max-time 8 --range 0-3 \
      "http://127.0.0.1:${INTERNAL_PORT}/static/7zr.bin" 2>/dev/null \
      | od -An -N4 -tx1 2>/dev/null \
      | tr -d '[:space:]' \
      || true
  )"

  [[ "${HEALTH_FRONTEND_CODE}" =~ ^[23][0-9][0-9]$ \
      && "${HEALTH_API_CODE}" == "200" \
      && "${HEALTH_STATIC_CODE}" == "200" \
      && "${HEALTH_STATIC_SIGNATURE}" == "7f454c46" ]] \
...
wait_for_http() {
  ...
  die "Hashtopolis did not become healthy within 420 seconds (frontend_http=${HEALTH_FRONTEND_CODE}, agent_api_http=${HEALTH_API_CODE}, static_7zr_http=${HEALTH_STATIC_CODE}, static_7zr_signature=${HEALTH_STATIC_SIGNATURE:-missing})."
}
